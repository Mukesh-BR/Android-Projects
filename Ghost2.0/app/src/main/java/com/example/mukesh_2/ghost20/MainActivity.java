...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cGhost = (TextView) findViewById(R.id.textView);
        pGhost = (TextView) findViewById(R.id.textView2);
        ...
        if(input.equals("")) {
            ...
            if(!(dictionary.isWord(newText))) {
                if(dictionary.isBeginOFSomeWord(newText)) {
                    String compLetter;
                    while(true) {
                        compLetter = dictionary.bestLetterChoice(newText);
                        if(dictionary.isBeginOFSomeWord(newText + compLetter)) {
                            break;
                        }
                    }
    ...
    {
        TextView ti = (TextView) findViewById(R.id.textView4);
        ti.setText("Mukesh\nShanthanu\nKevin");
    }
    ...
    private void playerLose() {
        ...
        pGhost.setGravity(Gravity.CENTER);
        if(itP != ghost.length() - 1) {
            itP++;
        }
        reset();
